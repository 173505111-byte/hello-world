...
const themeAssets = {
  ...
  red: {
    character: "themes/red/character.png",
    label: "红发碎光主题"
  },
  pink: {
...
archiveNav.innerHTML = `
  <a href="#appShell" class="active" data-theme-target="pink"><b>01</b><span>PINK<br />粉</span></a>
  <a href="#appShell" data-theme-target="ice"><b>02</b><span>ICE<br />冰</span></a>
  <a href="#appShell" data-theme-target="red"><b>03</b><span>CRIMSON<br />绯</span></a>
`;
...
function renderFashionGallery() {
  ...
  filteredGalleryItems.forEach((galleryItem) => {
    ...
    button.innerHTML = `<img src="${src}" alt="${name}" />`;
...
themeNavLinks.forEach((link) => {
  link.addEventListener("click", (event) => {
    ...
    archiveLinks.forEach((item) => item.classList.remove("active"));
    updateThemeNavActive();
...
archiveLinks.forEach((link) => {
  link.addEventListener("click", (event) => {
    event.preventDefault();
    themeNavLinks.forEach((item) => item.classList.remove("active"));
...
function activateSection(target) {
  ...
  archiveLinks.forEach((item) => item.classList.toggle("active", item.dataset.section === target));
...
momentPhoto.addEventListener("change", () => {
  const files = Array.from(momentPhoto.files || []).slice(0, 9);
  if (!files.length) {
...
async function tryStartCoverMusic() {
  if (!document.body.classList.contains("has-cover") || !coverMusic.paused) return;
  try {
...
function applyTheme(theme, options = {}) {
  ...
  document.body.dataset.coverTheme = activeTheme;
  characterImage.src = themeAssets[activeTheme].character;
  characterImage.alt = themeAssets[activeTheme].label;
  characterImage.classList.remove("outfit-preview");
